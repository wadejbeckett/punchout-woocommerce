...
No stub exists for `wp_insert_user`, `wp_update_user`, `wp_delete_user`, `wp_create_user`, `wp_generate_password`, `update_user_meta`, `add_user_meta`, `delete_user_meta`, `add_role` or `remove_role`, and none may be added. The plugin never creates, renames or deletes users and never writes user meta on the bound account; an absent function fails loudly where a fake would quietly pass. A suite that genuinely needs to observe such a call defines a namespace-local seam that binds only inside itself (`Unit/CompanyBookTest.php`, `Unit/ExitPolicyTest.php`). `add_filter`, `remove_filter` and `add_action` are absent for a different reason: the global hook surface belongs to `fixtures/plugin-boot-master-off.php`, whose own `function_exists` guards would go dark if `wp-stubs.php` claimed those names, so suites that need to watch hook registration own them namespace-locally too. `Unit/HarnessPerVisitTest.php` pins all of this — the recorder, the per-request token, the multi-row session fixture and the absences.
