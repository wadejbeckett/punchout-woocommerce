...
t4_native_case('stale admin save cannot revive a disabled connection; pending edits require approval',function()use($r,$svc){$f=t4_native_fixture(true);t4_native_check($r->update($f['id'],['status'=>'active','to_identity'=>'edited'],'not-issued'));t4_native_check($r->find($f['id'])->is_pending()&&$r->find($f['id'])->secret_current==='');t4_native_check($svc->approve($f['id'],t4_native_admin())!=='');t4_native_check($r->transition_status($f['id'],'active',['status'=>'disabled'])===false);t4_native_check($r->with_partner_lock($f['id'],fn()=>$r->transition_status($f['id'],'active',['status'=>'disabled'])));t4_native_check($r->update($f['id'],['status'=>'active','name'=>'stale'],'not-issued'));$p=$r->find($f['id']);t4_native_check($p->status==='disabled'&&$p->secret_current==='');});
