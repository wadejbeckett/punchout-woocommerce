 ...
 *
 * The connection's fixture is one ordinary WooCommerce customer account and the visits of it: the lifecycle creates no user and the login rows it drains are visits, each with its own WP session token and its own per-visit cart key. Every actor check is a capability check on a real account — the account a connection is bound to is not an administrator and cannot approve, reset or rotate its own credential.
 *
 * @package POW
...
function t4_native_check($v){if(!$v){$c=debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS,1)[0];throw new RuntimeException('assertion at line '.$c['line']);}}
function t4_native_case($name,$fn){if('credential-recovery'===getenv('POW_NATIVE_CASES')&&!str_starts_with($name,'credential recovery ')&&!str_starts_with($name,'native lock release failure'))return;try{$fn();echo "PASS $name\n";$GLOBALS['pow_native_registration_pass']++;}catch(Throwable $e){echo "FAIL $name ".get_class($e).": ".$e->getMessage()."\n";$GLOBALS['pow_native_registration_fail']++;}}
function t4_native_fixture($pending=false){global $r,$svc; $suffix=bin2hex(random_bytes(6));$owner=pow_native_bound_account('fault-owner');$data=['name'=>'Fault fixture','status'=>'pending','owner_user_id'=>$owner,'from_domain'=>'NetworkID','from_identity'=>'buyer-'.$suffix,'sender_domain'=>'NetworkID','sender_identity'=>'buyer-'.$suffix,'to_domain'=>'NetworkID','to_identity'=>'supplier','company_profile'=>'{"book":"preserve"}'];$id=$r->insert($data);$secret=$pending?'':$svc->approve($id,t4_native_admin());return compact('id','owner','data','secret');}
/** One visit of the connection's bound account: its own login token, its own per-visit cart key, no user created. */
function t4_native_login_row($f,$status='active'){return pow_native_open_visit($f['id'],$f['owner'],['status'=>$status,'buyer_identity'=>'fault-'.bin2hex(random_bytes(5)).'@example.invalid']);}
...
foreach(['false','throw'] as $mode)t4_native_case('native token-map '.$mode.' write prevents activation and allows retry',function()use($mode,$r,$store,$svc){$f=t4_native_fixture();$row=t4_native_login_row($f,'returned');$unrelated=WP_Session_Tokens::get_instance($f['owner'])->create(time()+3600);$filter=function($check,$id,$key)use($f,$mode){if($id===$f['owner']&&$key==='session_tokens'){if($mode==='throw')throw new RuntimeException('native token write');return false;}return $check;};t4_native_check(t4_native_filtered('update_user_metadata',$filter,fn()=>$svc->reset($f['id'],$f['data'],t4_native_admin()),10,3)==='');t4_native_check($r->find($f['id'])->status==='disabled'&&$store->login_valid_checked($row));t4_native_check($svc->reset($f['id'],$f['data'],t4_native_admin())!==''&&!$store->login_valid_checked($row));t4_native_check(WP_Session_Tokens::get_instance($f['owner'])->verify($unrelated));});
t4_native_case('native token-map read error cannot confirm absent login',function()use($r,$store,$svc){$f=t4_native_fixture();$row=t4_native_login_row($f);$filter=function($q)use($f){if(str_contains($q,'meta_key, meta_value FROM')&&str_contains($q,'user_id IN ('.$f['owner'].')'))return 'SELECT * FROM t4_missing_meta';return $q;};t4_native_check(t4_native_filtered('query',$filter,fn()=>$svc->reset($f['id'],$f['data'],t4_native_admin()))==='');t4_native_check($r->find($f['id'])->status==='disabled');t4_native_check($svc->reset($f['id'],$f['data'],t4_native_admin())!==''&&!$store->login_valid_checked($row));});
t4_native_case('stale admin save cannot revive a disabled connection; pending edits require approval',function()use($r,$svc){
	$f=t4_native_fixture(true);
	t4_native_check($r->update($f['id'],['status'=>'active','to_identity'=>'edited'],'not-issued'));
	t4_native_check($r->find($f['id'])->is_pending()&&$r->find($f['id'])->secret_current==='');
	t4_native_check($svc->approve($f['id'],t4_native_admin())!=='');
	t4_native_check($r->transition_status($f['id'],'active',['status'=>'disabled'])===false);
	t4_native_check($r->with_partner_lock($f['id'],fn()=>$r->transition_status($f['id'],'active',['status'=>'disabled'])));
	// The administrator's disable-and-revoke: the only path that retires a connection now that owners cannot deactivate from My Account.
	t4_native_check($r->revoke_secret($f['id']));
	t4_native_check($r->update($f['id'],['status'=>'active','name'=>'stale'],'not-issued'));
	$p=$r->find($f['id']);
	t4_native_check($p->status==='disabled'&&$p->secret_current==='');});
...
t4_native_case('reset drains 505 mixed open and terminal visits across all batches',function()use($r,$svc,$store){
 $f=t4_native_fixture();$old=[];for($i=0;$i<505;$i++)$old[]=t4_native_login_row($f,$i<503?['pending','active','ordered'][$i%3]:['returned','closed'][$i-503]);
 $unrelated=WP_Session_Tokens::get_instance($f['owner'])->create(time()+3600);$before=$r->find($f['id']);
 t4_native_check($svc->reset($f['id'],$f['data'],t4_native_admin())!=='');
 $after=$r->find($f['id']);t4_native_check($after->is_active()&&$after->owner_user_id===$before->owner_user_id&&$after->company_profile===$before->company_profile&&$store->open_for_partner($f['id'],1000)===[]);
 foreach($old as $row)t4_native_check(!$store->login_valid_checked($row));t4_native_check(WP_Session_Tokens::get_instance($f['owner'])->verify($unrelated));
 foreach($old as $row)t4_native_check(pow_native_cart_row((string)$row->wc_session_key)===null);
});
t4_native_case('an incomplete visit drain issues no replacement credential and names the cleanup',function()use($r,$store,$log,$wpdb){
 ...
 t4_native_check(t4_native_filtered('pre_wp_mail',$capture,fn()=>$svc->reset($f['id'],$f['data'],t4_native_admin()),PHP_INT_MAX,2)==='');
 $reason=$wpdb->get_var($wpdb->prepare('SELECT detail FROM '.POW\Installer::log_table()." WHERE partner_id=%d AND event='registration_reset_failed' ORDER BY id DESC LIMIT 1",$f['id']));
 t4_native_check($r->find($f['id'])->status==='disabled'&&str_contains((string)$reason,'session_cleanup')&&!str_contains($message,$f['secret'])&&$r->verify_secret($r->find($f['id']),$f['secret'])===null);
});
t4_native_case('the bound account cannot approve, reset or rotate its own connection',function()use($r,$svc){$f=t4_native_fixture(true);wp_set_current_user($f['owner']);try{t4_native_check($svc->approve($f['id'],t4_native_admin())===''&&$svc->reset($f['id'],$f['data'],t4_native_admin())===''&&$svc->approve($f['id'],$f['owner'])==='');t4_native_check($r->rotate($f['id'])===null&&!$r->associate_owner($f['id'],$f['owner']));}finally{wp_set_current_user(t4_native_admin());}t4_native_check($r->find($f['id'])->is_pending()&&$r->find($f['id'])->secret_current==='');});
t4_native_case('administrator rotation overlaps until close; reset rejects both former secrets',function()use($r,$svc){$f=t4_native_fixture();$rotated=$r->rotate($f['id']);$p=$r->find($f['id']);t4_native_check($rotated!==null&&$r->verify_secret($p,$f['secret'])!==null&&$r->verify_secret($p,$rotated)!==null);t4_native_check($r->rotate($f['id'])===null&&$r->close_rotation($f['id']));t4_native_check($r->verify_secret($r->find($f['id']),$f['secret'])===null);t4_native_check($svc->reset($f['id'],$f['data'],t4_native_admin())!=='');$p=$r->find($f['id']);t4_native_check($r->verify_secret($p,$f['secret'])===null&&$r->verify_secret($p,$rotated)===null&&$p->secret_previous==='');});
t4_native_case('reset preserves historical native order and the bound account profile',function()use($r,$svc){$f=t4_native_fixture();update_user_meta($f['owner'],'billing_city','Fixture City');$order=wc_create_order(['customer_id'=>$f['owner']]);$order->set_total('25.75');$order->update_meta_data(POW\Orders\QuoteOrder::META_PARTNER_ID,$f['id']);$order->save();$id=$order->get_id();t4_native_check($svc->reset($f['id'],$f['data'],t4_native_admin())!=='');$fresh=wc_get_order($id);t4_native_check($fresh->get_customer_id()===$f['owner']&&$fresh->get_status()==='pending'&&$fresh->get_total()==='25.75'&&(int)$fresh->get_meta(POW\Orders\QuoteOrder::META_PARTNER_ID)===$f['id']&&get_user_meta($f['owner'],'billing_city',true)==='Fixture City');});
