...
Every cart return leaves a WooCommerce order behind, in a status of its own: **Punchout Quote** (`punchout-quote`). It carries exactly the lines, quantities and ex-tax prices the buyer's system was quoted, the delivery address, and the cXML basket that went back — it is created between the cart mapping and the basket build, so the order and the document cannot disagree about price. It is a record of what was quoted, not a sale: the requisition itself lives in the buyer's procurement system.
...
A failure never reaches the buyer: the basket still posts back, the failure is logged and audited (`quote_order_failed`), any part-built order is cancelled for you, and the administrator is emailed — at most once an hour, however many failures follow.

**Converting.** The order screen's *Order actions* box offers **Convert Punchout Quote to order** on a quote and on nothing else. It moves the order to the **Convert quotes to** status (Pending payment, Processing or On hold), leaves a note, and writes a `quote_order_converted` audit row naming the administrator who did it. Nothing else about the order changes.

**Retention.** The hourly housekeeping job cancels quotes nobody converted within **Quote retention** days — cancelled, never deleted, with a note saying why and a `quote_order_expired` audit row. The basket XML stays on the order, and the cancellation restores no stock, because a quote never reduced any. Set the retention to 0 to keep quotes indefinitely.

**Order meta.** Four keys, each underscore-prefixed and therefore protected: WooCommerce neither shows them in the order screen's custom-fields box nor lets anyone edit them there.
...
| `_pow_delivery_code` | The buyer's own code for the delivery address, when one was supplied |

The delivery address is settled in a fixed order: the `pow_quote_shipping_address` filter, then the `ShipTo` the buyer's system sent at setup, then the customer's saved shipping address.

## Integration documentation page

Everything a buyer's developer needs to connect — endpoints, identities, the annotated setup request, the cart we send back, every status code and what to do about it, delivery-address fields, rate limits, secret rotation, and what is deliberately not supported. The samples are generated by the same `Cxml\Parser` and `Cxml\Builder` that serve live traffic, so the page cannot document a document the plugin does not actually send.
...
| `pow_delivery_codes_enabled` | filter | Whether the documentation page presents delivery codes as live rather than forthcoming |
...
| `pow_quote_shipping_address` | filter | The delivery address stamped on the Punchout Quote order (`null, $session, $partner`) |
