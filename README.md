...
└── includes/
    ...
    ├── Account/VisitEndpoints.php  The My Account pages a visit may open: the form's groups, the dashboard entry, the payment rule
...
**The list.** The connection's edit screen has a checkbox list, **My Account pages in a visit**, built live from the account endpoints WooCommerce has registered (`WC()->query->get_query_vars()`). Each row shows the page's title where WooCommerce gives one, its endpoint name and a note. The rows are grouped: the dashboard (the account page itself, `/my-account/`) and WooCommerce's own pages first, then this plugin's Punchout integration tab, then **Added by other plugins**, which is every endpoint that is not one of WooCommerce's own. Some plugins register a page only for some accounts or only on the front end, so a page the connection already lists can be missing when the screen loads. It then appears under **Stored, not currently registered**, still ticked, so saving the form keeps it; untick it to remove it. The list is edited by the same users, with the same capability check, as every other connection field.

The notes say what sharing a page means:

- Orders, order view and downloads are shared by every buyer of the connection.
...
- In red: log out and lost password break the punchout visit, the Punchout integration tab exposes the connection's settings, and order-pay and order-received are checkout pages. Checkout stays refused in every visit whatever is ticked.
