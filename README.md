...
└── includes/
    ...
    ├── Account/VisitEndpoints.php  The My Account pages a visit may open, and the fixed hard-deny list
...
| Table | Holds |
|---|---|
| `wp_pow_partners` | Company-connection registry: identities, sealed secrets (current+previous), cXML version, deployment mode, return encoding, delivery configuration, ALL-CAPS flag, IP allowlist, the bound customer account every buyer of this connection punches in as (`owner_user_id`), TTLs, the My Account endpoints its visits may open (`visit_endpoints`). The open-visit cap is a constant, not a column |
...
Every buyer of a connection is signed in as its one account, so the account's own My Account pages are every colleague's at once. Inside a visit they stay closed: the dashboard, orders and order view, downloads, addresses, account details, payment methods, password reset, logout, the Punchout integration tab, and WooCommerce's order-pay and order-received endpoints, which have no account content and would show the dashboard. That list is fixed in code (`Account\VisitEndpoints::HARD_DENY`) and cannot be changed.

Other My Account pages, typically third-party ones such as a quick-order form, are closed too unless the connection lists them. Enter their endpoint names, comma-separated and lowercase, in **My Account pages in a visit** on the connection's edit screen, for example `bulkorder`. The field is empty by default. A save that names a hard-denied page, or anything that is not an endpoint slug, is refused with a notice naming the entry. Inside a visit a listed page opens only when every endpoint the request names is listed and has account content of its own (a `woocommerce_account_<endpoint>_endpoint` action) on that request. Without content WooCommerce would show the dashboard in its place, so such a page stays closed and leaves the menu. A name that is not a registered endpoint therefore opens nothing, and neither does an endpoint whose plugin shows its content only to some accounts until the bound account is one of them. The check runs at the start of `template_redirect` and again at its end. The account menu shows only the listed pages that open. The account holder sees the list, read-only, on **My Account → Punchout integration**.
