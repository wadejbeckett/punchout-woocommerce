...
Paste the XML into the buyer system's cXML template editor and replace only the SharedSecret placeholder. The downloaded payloadID, timestamp, BuyerCookie and BrowserFormPost/URL fields are blank by design. Configure them through the buyer product's documented runtime controls. Configure UserEmail separately through its extrinsics mapping; the downloaded XML does not duplicate that element. BrowserFormPost is the buyer's cart-return receiver, not the supplier setup URL. Do not invent substitution tokens.
