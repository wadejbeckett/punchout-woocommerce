...
5. Confirm the item-7 refusals inside a visit: wp-admin, `/wp/v2/users*` and application passwords are refused, and so is every My Account page the connection has not ticked, account details, password change and the punchout tab included. Then check the connection's **My Account pages in a visit**: confirm each ticked page opens and shows its own content, the dashboard opens only when it is ticked, the account menu shows only the ticked pages, an unticked page such as orders sends the buyer to the landing page, and nothing on a ticked page creates a user. With the dashboard ticked, confirm it shows no "temporary password" notice or Resend link and no "Log out" link in a visit. If a page another plugin adds is missing from the list in wp-admin, add it with **Add a page by name** below the list.
