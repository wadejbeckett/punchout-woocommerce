...
5. Confirm the item-7 refusals inside a visit: wp-admin, `/wp/v2/users*`, application passwords, account details and password change are all refused, as is the My Account punchout tab. If the connection lists My Account pages, confirm each listed page opens and shows its own content, not the account dashboard, the account menu shows only those pages, the dashboard and orders still send the buyer to the landing page, and nothing on a listed page creates a user.
